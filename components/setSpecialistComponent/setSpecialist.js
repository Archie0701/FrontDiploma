...
import { getImageById, fetchUserData, fetchProposalData, acceptProposal, declineProposal, criterias, addComment } from '../../services/apiService';
...
function SetSpecialist(props) {
  ...
  const fetchData = async () => {
    try {
      ...
      const proposalDataResponse = await fetchProposalData();
      const criteriasData = await criterias();

      const filteredProposalData = proposalDataResponse.filter(item => {
        return item.specialist !== null;
      });
      filteredProposalData.forEach(item => {
        console.log(item.specialist);
      })
      console.log("Filtered proposal data", filteredProposalData);
  ...
  const updateProposalList = async () => {
    try {
      const updatedProposalData = await fetchProposalData();
